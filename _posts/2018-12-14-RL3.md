...
记迭代轮数为$k$,基本想法就是用上一轮的状态价值函数结合(1.1)式算出当前轮的状态价值函数。即
...
1. 在该空间中存在唯一的点$x_*$满足$f(x_*)=x_*$
2. 对该空间中的任意点$x$，有$\lim\limits_{n \to \infty}f^n(x)=x_*$。其中$f^n(x)=f(f^{n-1}(x))$
****
##### 压缩映射定理的证明
...
#### 迭代收敛正确性的证明
记$$ \mathcal{T}（\mathcal{V}^{\pi})=\mathcal{R}^{\pi}+\gamma \mathcal{P}^{\pi} \mathcal{V}^{\pi} \tag{6}$$,则我们每一轮的状态函数值就是数列$\{\mathcal{T}^k(\mathcal{V}^{\pi})\}$。下面我们要说明$\mathcal{T}$是个压缩映射。首先需要定义一个距离，这里我们采用无穷范数。矩阵的无穷级数是行和的绝对值的最大值，而向量的无穷级数则是单个元素的绝对值的最大值。
...
对任意状态$s$,$$\pi^{'}(s)=argmax_{a}[q_{\pi}(s,a)]$$
...
\leq r_s^{\pi^{'}(s)}+\gamma \sum_{s' \in S}P(s_{t+1}=s'|s_t=s,a_t=\pi^{'}(s))\cdot [r_{s'}^{\pi^{'}(s')}+\gamma \sum_{s'' \in S}P(s_{t+2}=s''|s_{t+1}=s',a_{t+1}=\pi^{'}(s')) \cdot v_{\pi}(s'')]\\
\leq ... \leq v_{\pi^{'}}(s)$$
